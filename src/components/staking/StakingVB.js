...
import BtnCooldownAndUnstake from "./BtnCooldownAndUnstake";
import BtnOpenClaimStaked from "./BtnOpenClaim";

import CurrencyAssetsUSD from "../markets/CurrencyAssetsUSD";

const StakingVB = ({ loadStakingAccount, addressStaking}) => {

  const tokenStaking = process.env.REACT_APP_TOKEN_VEBANK;
  
  const { accountStaked, accountRewards, stakedAPR, perMonth, loading } = useSelector((state) => state.accountStakedVBReducer,shallowEqual);
      ...
      <div className="flex flex-col space-y-[6px]">
        ...
        <div className="flex flex-row space-x-[6px]">
          <div className="flex flex-col w-1/2 p-6 items-center justify-center bg-[#182844] rounded-md">
            ...
            <BtnCooldownAndUnstake addressStaking={addressStaking} />
            ...
            <div className="flex flex-row w-full items-center justify-between mt-4">
              ...
              <CooldownPeriod loadStakingAccount={loadStakingAccount} />
            </div>
          </div>
          <div className="flex flex-col w-1/2 p-6 items-center justify-center bg-[#182844] rounded-md">
            <span className="font-poppins_medium text-base text-white">
