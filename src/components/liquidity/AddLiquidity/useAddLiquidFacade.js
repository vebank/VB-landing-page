...
const useAddLiquidFacade = () => {
  ...
  const refInputToken = { refInputFirstToken, refInputSecondToken };
    ...
    (value) => {
      ...
      if (value.isMatch?.(/^\d*\.?\d*$/)) {
        if (totalSupply === 0) {
          ...
          let fixed = firstTokenInfo?.assetsSymbol === "VEUSD" ? 6 : 18;
          if (temp[1]) {
    ...
    (value) => {
      ...
      if (value.isMatch?.(/^\d*\.?\d*$/)) {
        if (totalSupply === 0) {
          ...
          let temp = value.split(".");
          let fixed = secondTokenInfo?.assetsSymbol === "VEUSD" ? 6 : 18;
          if (temp[1]) {
            if (temp[1].length > fixed) {
              setSecondTokenVolume(value);
              setFirstTokenVolume("");
  ...
  const resetFrm = () => {
    ...
    setSecondTokenVolume("");
    setSlippage("0.5");
    refInputFirstToken?.current?.focus();
    setStep(1);
  };
  ...
  const onChangeSlippage = (value) => {
    ...
    setContinueAvailable(Number(value) >= 0.5);
    // let pattern = /^\d+\.?\d*$/;
    let pattern = /^\d*(\.\d{0,2})?$/;
  ...
  useEffect(() => {
    ...
    } else if (slippage < 0.5 || slippage === "") {
      setContinueAvailable(false);
      setPrimaryButtonLabel(
        "Slippage no smaller than 0.5% or no greater than 50%"
      );
    } else {
      setContinueAvailable(true);
      dispatch(
  ...
  useEffect(() => {
    ...
    if (!poolAddress) {
      dispatch(
        actions.selectDefaultFirstToken(process.env.REACT_APP_TOKEN_WVET)
      );
      dispatch(
        actions.selectDefaultSecondToken(process.env.REACT_APP_TOKEN_VEBANK)
      );
    }
